# Stackroom — School Library System (v2.2, feature update)
...
> **v2.2 update (this revision):** see "What changed in v2.2" further
> down for the newly added Student Bulk QR generator, multi-file
> encrypted backup merge, Student QR scan during Issue Book, the
> multiple-books special permission, automatic first-time Recovery Code
> setup, and the exactly-one-beep scanner audio feedback. Everything in
> the rest of this README describes the earlier v2 storage/security
> upgrade and still applies unchanged.
...
This revision layers six requested features onto the app above, using the
same pattern as before: extend existing functions/data, add new ones
alongside them, never rewrite or remove working code.

### 1. Recovery Code — automatic first-time setup
`recoveryConfigured()` is now checked right after a brand-new Admin PIN is
created; if no Recovery Code exists yet, the setup modal opens
automatically at that moment instead of requiring a manual trip to
Settings. Guarded so it never reappears once a code exists — including on
a later "Change Admin PIN", on login, or on refresh.

### 2. Student Bulk QR Code generator
New "Book Information" / "Student Information" toggle inside Tools → Bulk
QR. The Book generator is untouched; the Student side reuses its exact
architecture (`qrToPngDataUrl`, `drawScissors`, A4/5×5/25-per-page layout):
- Header-alias-tolerant Excel import (`Student ID / Roll Number`,
  `Student Name`, `Class / Section` — case/whitespace tolerant).
- Per-row validation with visible skip reasons; one bad row never stops
  the rest.
- QR payload is versioned and distinguishable from a Book QR:
  `{type:"student", version:1, studentId, studentName, class, section}`.
  Book QR payloads never carry a `type` field, so detection is
  unambiguous and backward compatible.
- "Download Sample Student Excel" alongside the existing Book sample
  download (both remain available).

### 3. Multiple encrypted backup import + merge
`importFile` now accepts multiple files. `runMultiBackupImport()`
decrypts each independently with one entered password — a corrupted,
wrong-password, or non-Stackroom file fails on its own without blocking
the rest. Duplicate detection (by barcode/Book ID, Student ID, and
transaction ID) is cumulative across the whole batch, and existing
records are **never** overwritten — this is always import+merge, never
restore+replace. A scrollable report lists every skipped record with its
file name, type, ID and reason.

### 4. Student QR scan during Issue Book
Stage 1 of Issue Book now has a Manual Entry / Scan Student QR toggle.
Scanning fills Student ID, Name, and Class/Section automatically; manual
entry is unchanged and still the default. A shared payload-type detector
(`parseQrPayload`) means a Book QR scanned in the student slot (or a
Student QR scanned in any book slot — Add Book, Issue, Return) shows a
plain "Please scan a Book/Student QR Code" message instead of silently
corrupting fields.

### 5. Multiple-books special permission
Student records gained an `allowMultipleBooks` field (defaults to
`false` for every existing and new student — no migration needed, since
Dexie doesn't enforce field-level schema). A checkbox in Issue stage 1
persists it per Student ID. Both the stage-1 warning **and** the final
check immediately before transaction creation now respect it, so a
double-click, stale tab, or race condition still can't create a second
active loan for a student without the permission. Disabling the
permission later never touches existing active loans — only blocks new
ones.

### 6. Scanner audio feedback — exactly one beep
A single Web Audio beep is wired into the one shared decode callback
inside `startScanner()` and into `decodeQrImageFile()` — meaning every
scanner in the app (Add Book, Issue, Return, Student QR, image upload)
gets it from one place. The existing `scanProcessed` duplicate-callback
guard already prevents a second beep for the same physical scan.

### Files touched this revision
`Stackroom_Library_System.html` (all logic above), `sw.js` (cache
version bumped to `stackroom-v3` since cached app content changed).
`manifest.json` and both icons are unchanged.

### Verification performed this revision
- Full-file JS syntax check (`new Function()` on the extracted script) —
  clean.
- Duplicate DOM `id` scan across the whole file — none found.
- Every `getElementById(...)` call cross-checked against actual HTML
  `id`s — all resolve.
- Every newly added function confirmed to be called from somewhere
  (no dead code) via reference-count scan.
- Every reused CSS class (`.scanner-toggle`, `.table-wrap`,
  `.bulk-row`, etc.) confirmed to already exist in the stylesheet, not
  just invented in the new markup.
- Caught and fixed one real regression during review: a pre-existing
  scanner-toggle click handler was scoped to `#view-issue
  .scanner-toggle button`, which would have also matched the new
  Student-scan toggle buttons and cross-wired their behavior with the
  book-scan toggle. Rescoped to `#issueStage2 .scanner-toggle button`.

### Known limitations (v2.2, in addition to the v2 list above)
- No live browser/camera/PDF run was performed on this revision either
  — verification above is static (syntax, references, structure), not
  a functional browser test. Please smoke-test camera scanning (all
  four scan points), the Student QR PDF print layout, and a real
  multi-file backup import/merge on an actual device before relying on
  this in production.
- The multiple-books permission has no dedicated Student Registry
  screen to manage in bulk — it's set per student from the Issue Book
  screen, since the app has no standalone student-management view to
  attach it to. If you'd like a dedicated Students view added, that's a
  separate, larger change I did not make unprompted.
