...
## ⚠️ Issue Book workflow + Service Worker update fix (latest round) — read this first

This round addressed three reported problems in `index.html`/`sw.js`: the
"Issue Book" button appearing to go missing, scans not auto-advancing to
the next step, and users seeing stale cached versions after a deploy. A
full manual test checklist for this round is in
`MANUAL_TEST_CHECKLIST.md` — **run it before trusting this in
production**; see the testing-limitation note below for why it wasn't
pre-run here.

**Root causes found (by code tracing, not guessing):**
- **"Issue Book" button missing** — the button itself was always
  correctly wired to Stage 3 of the Issue Book wizard. The real defect
  was one step earlier: a successful **Student QR scan** filled in the
  form fields and then just stopped, requiring the operator to notice
  and manually click "Continue to book scan" before Stage 2 (and
  eventually Stage 3, where the button lives) was ever reached. An
  operator scanning student→book in one continuous motion — the natural
  way to use a QR workflow — could get stuck on Stage 1 indefinitely,
  which reads from the outside as "the button never appears."
- **No auto-advance between scans** — same root cause: only the *book*
  scan auto-advanced (to the confirm stage, after a 500ms pause); the
  *student* scan did not auto-advance to the book-scan stage, and the
  book-scan camera wasn't auto-started either.
- **Stale cached version after deploy** — `sw.js`'s versioned-cache and
  activate-time cleanup logic (`CACHE_VERSION`, deleting old
  `stackroom-*` caches) was already sound. The gap was in `index.html`:
  the Service Worker was registered with **no update-detection at all**.
  Because `sw.js` calls `skipWaiting()`/`clients.claim()`, a newly
  deployed Service Worker silently takes control of already-open tabs —
  but the JS/DOM that tab already has loaded in memory is untouched
  until an actual page reload, and nothing told the admin a reload was
  what they needed.

**Changes made:**
- `index.html` — extracted the student-eligibility check (missing
  fields, one-active-loan-per-student rule) into a shared
  `proceedToBookScan()` function, called both by the existing "Continue
  to book scan" button (manual entry) and automatically right after a
  successful, valid Student QR scan. On auto-advance, also
  auto-starts the book-scan camera if it's in Camera mode, so the
  operator doesn't need to tap "Start Camera" either. If a scan is valid
  but blocked by the one-loan rule, the camera is kept running (with its
  one-decode-per-scan guard correctly reset) so a different, eligible
  student can be scanned immediately without a page reload.
- `index.html` — added Service Worker update detection
  (`updatefound`/`statechange` listeners) that shows a dismiss-by-action
  **"Update available — Reload now"** toast instead of forcing an
  unannounced reload, so an admin mid-transaction is never interrupted
  without warning.
- `index.html` — `doLogout()` now checks whether an update is already
  pending and, if so, reloads the page as part of logging out (safe,
  since no authenticated session or in-progress workflow exists at that
  point); otherwise it triggers a background `reg.update()` check.
  **This never touches IndexedDB** — no book/student/transaction record,
  encryption key, or setting is read, written, or cleared by this path;
  it only reloads the document.
- `sw.js` — bumped `CACHE_VERSION` to `stackroom-v7`, per this file's
  own documented convention ("bump on every release that changes cached
  assets"), since `index.html` changed. No other line in `sw.js`
  changed — install/activate/fetch handlers, `RUNTIME_HOSTS`, and
  `SHELL_ASSETS` are byte-for-byte the same as before.

**All existing logic this round did not touch, and confirmed
unchanged** (verified by diffing against the previous file, not just by
intent): multi-book permission checks, the authoritative re-check inside
`issueConfirmBtn`'s handler immediately before a transaction is created,
issue/return date validation, `persistData()`'s atomic write, the audit
log, wrong-QR-type guards, the duplicate-decode guard, PIN/PBKDF2 auth,
session-token expiry/rotation, and Master Reset.

**⚠️ Testing limitation this round.** This session's sandbox had **no
network egress at all** — `cdn.jsdelivr.net`/`cdnjs.cloudflare.com`
returned HTTP 403 — so Dexie, html5-qrcode, jsPDF, and XLSX could not be
loaded, and unlike the previous round (below), no local Dexie-shim
harness was built this time. What **was** verified with a real Chromium
instance (via Playwright) was DOM/CSS-only: that the Issue Book wizard's
three stages transition cleanly via the `hidden` attribute with nothing
else in the stylesheet fighting it, that the "Issue Book" button renders
at a real, clickable size once Stage 3 is reached, that the update-toast
markup renders correctly, and that the full inline script still parses
(`node --check`) after every edit. **The Dexie-backed transaction write,
real camera QR decoding, and a real deploy→update→reload cycle were NOT
exercised this round** — see `MANUAL_TEST_CHECKLIST.md` for exactly what
to verify yourself, and why.
