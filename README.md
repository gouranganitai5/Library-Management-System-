# Stackroom — School Library System (v4, Field-Level Encryption edition)

## What changed in this round

You asked for exactly the thing the v3 note below flagged as impossible
under the old requirements — field-level AES-GCM encryption of IndexedDB,
**and** the PIN required again on every refresh. Because you're now
asking for both together, there's no more contradiction, and this round
implements the real thing.

### How field-level encryption works
Every sensitive record — books, transactions, students, settings, audit
log entries — is encrypted with a single random **Data Encryption Key**
(DEK, AES-256-GCM) before it's written to IndexedDB. Each row keeps only
its primary key in the clear (so Dexie can still get/put/delete it) plus
one field, `enc: { v, iv, ct }` — a fresh random IV and the AES-GCM
ciphertext of everything else about that record. Open DevTools → IndexedDB
Application; a book, a transaction, a student — none of it is readable.
Tampering with a ciphertext (or guessing wrong) is detected: AES-GCM's
authentication tag makes `crypto.subtle.decrypt` throw rather than ever
returning corrupted-but-plausible data.

### How the PIN unlocks the data
The DEK itself is never stored in the clear. It's *wrapped* (encrypted)
under a Key-Encryption Key that's derived from the admin's PIN via
PBKDF2-SHA256 (210,000 iterations, a fresh random 16-byte salt) — the PIN
is used only transiently to derive that KEK and is never itself written
anywhere. The wrapped DEK (ciphertext + salt + IV + iteration count) is
the only thing persisted, in `meta.encDekByPin`. On login: PIN entered →
`verifyAuthPin()` confirms it against the existing PBKDF2 verifier
(unchanged) → the SAME PIN re-derives the KEK → the DEK is unwrapped into
the in-memory `sessionDEK` variable → every protected screen can now
decrypt. Get the PIN wrong and the unwrap fails closed (or is never
attempted, since `verifyAuthPin` already rejected it) — nothing is ever
decrypted with an incorrect key.

A second, independent wrapping of the *same* DEK exists under the
recovery code (`meta.encDekByRecovery`), so **Forgot PIN → reset via
recovery code** can restore access to the encrypted data without ever
knowing the old PIN — it unwraps the DEK via the recovery code, then
re-wraps that same DEK under the newly chosen PIN. One real, disclosed
limitation: a recovery code generated by an older version of this app
was only ever stored as a one-way hash (by design, so it couldn't be
stolen from the database either), so there's no way to retroactively
wrap the DEK with it. Settings → Security now shows "regenerate to work
with encryption" for exactly this case, and regenerating immediately
fixes it. Until then, such a recovery code can still prove identity but
can't itself restore data access — the same real-world limitation as any
password + recovery-key scheme where the recovery key predates the data
it would need to protect.

Changing the PIN while already logged in re-wraps the *same* DEK under
the new PIN (the DEK itself never changes, so nothing needs re-encrypting).
Master Reset explicitly preserves the wrapped DEK entries alongside the
PIN/recovery verifiers — only library data is erased, so decryption keeps
working for whatever's written after the reset.

### How refresh / re-authentication works now
This is the opposite of what v3 shipped, on purpose, per your explicit
request. The old version kept a random session token in
`sessionStorage` + a matching `meta` row specifically so a refresh
wouldn't ask for the PIN again — that mechanism is now completely
removed. `sessionDEK` lives only in a plain JS variable, is set on
successful unlock, and is nulled on Sign Out — and, because it's just a
JS variable, is *implicitly* wiped the instant the page is refreshed,
navigated away, or closed (there's no way to "shred" freed memory from
JS beyond that; this is the realistic ceiling on client-side key hygiene
in a browser-only app). Every fresh page load — refresh or full
restart — always starts at the login screen and always requires the PIN
again before any protected view is reachable, because there is nothing
left anywhere that could skip that step.

### How existing IndexedDB data is migrated
`migratePlaintextToEncrypted()` runs once, right after the first
successful unlock post-upgrade (it needs the DEK, which needs a
successful login, so it can't run any earlier). It reads every row
across books/transactions/students/settings/auditLog, encrypts any row
that doesn't already have an `.enc` property, and `bulkPut`s the
encrypted versions back at the same primary keys — all inside one Dexie
`'rw'` transaction, so it either fully commits or fully rolls back (if
the browser is closed mid-migration, nothing is left half-encrypted; it
simply retries in full on the next login). It's idempotent two ways at
once: a `meta.fieldEncryptionVersion` marker short-circuits it entirely
once done, and independently, only rows still missing `.enc` are ever
touched — so even a failed marker write can never cause double
encryption or data loss on retry. Nothing is deleted; a plaintext row is
only ever *replaced* at its own key by its encrypted equivalent.

### Security limitations that remain (stated plainly, not glossed over)
- **The DEK is held as an *extractable* CryptoKey during a session** —
  a deliberate trade-off so PIN-change and recovery-code regeneration
  can re-wrap it via `crypto.subtle.wrapKey` without a separate copy of
  raw key bytes floating around in a plain variable. The exposure this
  adds is limited to code already running inside this page's own JS
  realm, which at that point already has direct access to every
  decrypted record anyway.
- **This is still a browser-only, client-side app.** Anyone with the
  correct PIN (or, transiently, code running in the same page) can read
  everything — that's inherent to the architecture and isn't something
  any client-side encryption scheme changes. What this implementation
  *does* protect against: someone with only raw access to the IndexedDB
  files (DevTools, browser profile, a copied `.sqlite`/leveldb store) —
  or a copy of that database moved to another browser entirely, per your
  requirement 13 — gets ciphertext, not readable library data, without
  the PIN or recovery code.
- **JS cannot truly zero memory.** `sessionDEK = null` and dropping
  local variables drops *references*; whether the underlying memory is
  actually overwritten before garbage collection reclaims it is up to
  the browser engine, not something JavaScript can force. This is
  standard for browser-only crypto and is why the DEK is nulled
  immediately on logout rather than being kept "just in case."
- **A pre-existing recovery code, from before this upgrade, cannot
  unwrap the new DEK** until it's regenerated (see above) — flagged
  explicitly rather than silently leaving a dead end.

---

## Previous round (v3, Fine/Grace + Master Reset edition)
